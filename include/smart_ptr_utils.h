 ...
 * ID          : 00991588
 * CLASS       : CS008
 * HEADER      : smart_ptr_utils
 * DESCRIPTION : This header provides tempalated smart pointer utility
 *      functions to manipulate arrays.
 ******************************************************************************/
#ifndef SMART_PTR_UTILS_H
#define SMART_PTR_UTILS_H
...
// U entry is the dereferenced item of shared_ptr
template <typename T, typename U>
std::size_t first_ge(const T* data, std::size_t size, const U& entry);
...
void ordered_insert(T* data, std::size_t& size, const T& entry, bool replace) {
    ...
    if(!size)
        data[size] = entry;
    else if(*entry > *data[size - 1])  // if greater last, insert after last
        data[size] = entry;
    else {
        while(forward < size) {
